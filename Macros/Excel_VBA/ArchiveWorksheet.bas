...
'--------------------------------------------
    ...
    NewName = WorkSheetName & "_" & Format(Now, "yyyy mmm dd")
    ...
    Application.CutCopyMode = False    'Remove values from clipboard so we do not paste it anywhere else by mistake.
    
    '8. Disable or delete buttons, Cycle through the controls on ws, these include the button
    For Each shp In wsNew.Shapes
        If shp.Type = msoFormControl Then	'Test if it is a control
            If shp.FormControlType = xlButtonControl Then  'Test if it is the button
